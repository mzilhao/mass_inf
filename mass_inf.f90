...
program mass_inflation
  ...
  double precision, dimension(NN) :: hp1, hp2, hp3, hp4, grad
  ...
  call init_cond( Du, Dv, u0, v0, mass, uf, vf, Nu, Nv, gradmax, gradmin, AMR, resu, resv, 10)

  allocate( plus(big_dim), minus(big_dim) )
  allocate( u(big_dim) )
  allocate( h_v0_new(NN, big_dim) )
